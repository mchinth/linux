 ...
 *
 */
static VOID sys_info_Get_Cpuid_Entry_Count(PVOID buffer)
{
	...
	if (is_function_1f_available) {
		...
		if (multi_die) {
			num_dies_per_pkg =
				(U32)1
				<< (shift_nbits_pkg -
				    shift_nbits_module); // num_dies in a pkg = 2 ^ (CPUID bit width for dies)
			package_id =
 ...
 *
 */
static VOID sys_info_Build_Percpu(VOID *buffer)
{
