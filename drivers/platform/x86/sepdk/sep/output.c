 ...
 */
static VOID
output_Free_Buffers(BUFFER_DESC buffer, size_t size)
{
	int    j;
	OUTPUT outbuf;
 ...
 */
extern void *
OUTPUT_Reserve_Buffer_Space(BUFFER_DESC bd,
			    U32         size,
			    DRV_BOOL    defer,
			    U8          in_notification)
{
 ...
 */
static int
output_Buffer_Fill(BUFFER_DESC bd, PVOID data, U16 size, U8 in_notification)
{
	char *outloc;
 ...
 */
extern int
OUTPUT_Module_Fill(PVOID data, U16 size, U8 in_notification)
{
	int    ret_size;
	    ...
	    BUFFER_DESC  kernel_buf)
{
	...
	if (to_copy == 0) {
		...
		SEP_DRV_LOG_TRACE("Decremented flush_writers. flush_writer=%u",
				  atomic_read(&flush_writers));
		if (flush_val == TRUE) {
			SEP_DRV_LOG_TRACE("Wake up flush_queue");
 ...
 */
extern ssize_t
OUTPUT_Module_Read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	ssize_t res;
...
/* ------------------------------------------------------------------------- */
/*!
 *  @fn  ssize_t  OUTPUT_IPT_Trace_Read(struct file  *filp,
 *                                             char         *buf,
 *                                             size_t        count,
 *                                             loff_t       *f_pos)
 *
 *  @brief  Return a processor trace buffer to user-mode. If not full or flush, wait
 *
 *  @param *filp   a file pointer
 *  @param *buf    a sampling buffer
 *  @param  count  size of the user's buffer
 ...
 */
extern ssize_t
OUTPUT_IPT_Trace_Read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	int     i;
	ssize_t size = 0;
	SEP_DRV_LOG_TRACE_IN("");
	...
	SEP_DRV_LOG_TRACE("Read request for ipt trace on minor %d.", i);
	size = output_Read(filp, buf, count, f_pos, &(ipt_trace_buf[i]));

	SEP_DRV_LOG_TRACE_OUT("size: %u.", (U32)size);
	return size;
}

/* ------------------------------------------------------------------------- */
/*!
 *  @fn  ssize_t  OUTPUT_IPT_Info_Read(struct file  *filp,
 *                                             char         *buf,
 *                                             size_t        count,
 *                                             loff_t       *f_pos)
 *
 *  @brief  Return a ipt info buffer to user-mode. If not full or flush, wait
 *
 *  @param *filp   a file pointer
 *  @param *buf    a sampling buffer
 *  @param  count  size of the user's buffer
 ...
 */
extern ssize_t
OUTPUT_IPT_Info_Read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	int     i;
	ssize_t size = 0;
	SEP_DRV_LOG_TRACE_IN("");
	...
	i = iminor(filp->DRV_F_DENTRY->d_inode); // kernel pointer - not user pointer
	SEP_DRV_LOG_TRACE("Read request for ipt trace on minor %d.", i);
	size = output_Read(filp, buf, count, f_pos, &(ipt_info_buf[i]));
 ...
 */
extern ssize_t
OUTPUT_Sample_Read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	int     i;
	ssize_t res;
 ...
 */
extern ssize_t
OUTPUT_UncSample_Read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	int     i;
	ssize_t res = 0;
 ...
 */
extern ssize_t
OUTPUT_SidebandInfo_Read(struct file *filp,
			 char        *buf,
			 size_t       count,
			 loff_t      *f_pos)
{
 ...
 */
extern ssize_t
OUTPUT_Emon_Read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	ssize_t res;
 ...
 */
static BUFFER_DESC
output_Initialized_Buffers(BUFFER_DESC desc, U32 factor)
{
	OUTPUT outbuf;
	int    j;
 ...
 */
static void
output_NMI_Sample_Buffer(unsigned long data)
{
	U32    cpu_id;
	OUTPUT outbuf;
	...
	if (ipt_trace_buf) {
		outbuf = &BUFFER_DESC_outbuf(&ipt_trace_buf[cpu_id]);
		if (outbuf && OUTPUT_signal_full(outbuf)) {
			wake_up_interruptible_sync(
				&BUFFER_DESC_queue(&ipt_trace_buf[cpu_id]));
			OUTPUT_signal_full(outbuf)    = FALSE;
			OUTPUT_tasklet_queued(outbuf) = FALSE;
		}
	}
	...
	if (ipt_info_buf) {
		outbuf = &BUFFER_DESC_outbuf(&ipt_info_buf[cpu_id]);
		if (outbuf && OUTPUT_signal_full(outbuf)) {
			wake_up_interruptible_sync(
				&BUFFER_DESC_queue(&ipt_info_buf[cpu_id]));
			OUTPUT_signal_full(outbuf)    = FALSE;
			OUTPUT_tasklet_queued(outbuf) = FALSE;
		}
	}
...
OUTPUT_Initialize(void)
{
	...
	if (DRV_CONFIG_ipt_mode(drv_cfg)) {
		for (i = 0; i < GLOBAL_STATE_num_cpus(driver_state); i++) {
			unused = output_Initialized_Buffers(
				&ipt_trace_buf[i], IPT_TRACE_BUFF_SIZE);
			if (!unused) {
				OUTPUT_Destroy();
				SEP_DRV_LOG_ERROR_TRACE_OUT(
					"OS_NO_MEM (failed to create ipt trace output buffer %d!).",
					i);
				return OS_NO_MEM;
			}
			unused = output_Initialized_Buffers(&ipt_info_buf[i],
							    IPT_INFO_BUFF_SIZE);
			if (!unused) {
				OUTPUT_Destroy();
				SEP_DRV_LOG_ERROR_TRACE_OUT(
					"OS_NO_MEM (failed to create ipt info output buffer %d!).",
					i);
				return OS_NO_MEM;
			}
		}
	...
	for (i = 0; i < GLOBAL_STATE_num_cpus(driver_state); i++) {
		tasklet_init(&CPU_STATE_nmi_tasklet(&pcb[i]),
			     output_NMI_Sample_Buffer, (unsigned long)NULL);
	}
 ...
 */
static void
output_tasklet_waker(PVOID ptr)
{
	SEP_DRV_LOG_TRACE_IN("");
 ...
 */
static void
output_dummy_tasklet_handler(unsigned long dummy)
{
...
OUTPUT_Initialize_EMON(void)
{
	...
	if (!unused) {
		OUTPUT_Destroy();
		SEP_DRV_LOG_ERROR_TRACE_OUT(
			"OS_NO_MEM (failed to create EMON output buffers!).");
		return OS_NO_MEM;
	}
...
OUTPUT_Flush(VOID)
{
	...
	SEP_DRV_LOG_TRACE("before atomic_set to %d writers.",
			  writers + OTHER_C_DEVICES);
	...
	SEP_DRV_LOG_TRACE("after atomic_set to %d writers.",
			  writers + OTHER_C_DEVICES);
	...
	// Flush all data from the processor trace buffers
	if (DRV_CONFIG_ipt_mode(drv_cfg)) {
		for (i = 0; i < GLOBAL_STATE_num_cpus(driver_state); i++) {
			if (CPU_STATE_initial_mask(&pcb[i]) == 0) {
				continue;
			...
			outbuf = &BUFFER_DESC_outbuf(&ipt_trace_buf[i]);
			OUTPUT_buffer_full(outbuf,
					   OUTPUT_current_buffer(outbuf)) =
				OUTPUT_total_buffer_size(outbuf) -
				OUTPUT_remaining_buffer_size(outbuf);
			wake_up_interruptible_sync(
				&BUFFER_DESC_queue(&ipt_trace_buf[i]));

			outbuf = &BUFFER_DESC_outbuf(&ipt_info_buf[i]);
			OUTPUT_buffer_full(outbuf,
					   OUTPUT_current_buffer(outbuf)) =
				OUTPUT_total_buffer_size(outbuf) -
				OUTPUT_remaining_buffer_size(outbuf);
			wake_up_interruptible_sync(
				&BUFFER_DESC_queue(&ipt_info_buf[i]));
		}
	}
	...
	SEP_DRV_LOG_TRACE("Wait for buffers to empty");
...
OUTPUT_Destroy(VOID)
{
	...
	if (ipt_trace_buf) {
		...
		for (i = 0; i < n; i++) {
			outbuf = &BUFFER_DESC_outbuf(&ipt_trace_buf[i]);
			output_Free_Buffers(&ipt_trace_buf[i],
					    OUTPUT_total_buffer_size(outbuf));
		}
	}
	...
	if (ipt_info_buf) {
		...
		for (i = 0; i < n; i++) {
			outbuf = &BUFFER_DESC_outbuf(&ipt_info_buf[i]);
			output_Free_Buffers(&ipt_info_buf[i],
					    OUTPUT_total_buffer_size(outbuf));
		}
	}
...
/*
 *  @fn exern int OUTPUT_IPT_Fill (data, size)
 *
 *  @param     data - pointer to a buffer to copy
 *  @param     size - size of the buffer to cpu
 *  @returns   size of data copied
 *
 *  @brief   Start by ensuring that output buffer space is available.
 *  @brief   If so, then copy the input data to the output buffer and make the necessary
 *  @brief   adjustments to manage the output buffers.
 */
extern int
OUTPUT_IPT_Fill(char *data, U32 size)
{
	BUFFER_DESC bd;
	char       *outloc = NULL;
	U32         this_cpu;

	SEP_DRV_LOG_TRACE_IN("Data: %p, size: %u.");

	this_cpu = CONTROL_THIS_CPU();

	bd = &ipt_trace_buf[this_cpu];

	outloc = OUTPUT_Reserve_Buffer_Space(
		bd, size, (NMI_mode) ? TRUE : FALSE, !SEP_IN_NOTIFICATION);
	SEP_DRV_LOG_TRACE("outloc: %p", outloc);
	if (!outloc) {
		SEP_DRV_LOG_TRACE_OUT(
			"OUTPUT_Reserve_Buffer_Space returns NULL!");
		return 0;
	}
...
/*
 *  @fn exern int OUTPUT_IPT_Info_Fill (data, size)
 *
 *  @param     data - pointer to a buffer to copy
 *  @param     size - size of the buffer to cpu
 *  @returns   size of data copied
 ...
 */
extern int
OUTPUT_IPT_Info_Fill(char *data, U32 size)
{
	BUFFER_DESC bd;
	char       *outloc = NULL;
	U32         this_cpu;
