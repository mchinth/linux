...
UNC_COMMON_Add_Bus_Map(U32 uncore_did, U32 dev_node, U32 bus_no, U32 domain_no)
{
	...
	if (!UNC_PCIDEV_busno_list(&(unc_pcidev_map[dev_node]))) {
		...
		UNC_PCIDEV_domainno_list(&(unc_pcidev_map[dev_node])) =
			CONTROL_Allocate_Memory(entries * sizeof(S32));
		if (UNC_PCIDEV_domainno_list(&(unc_pcidev_map[dev_node])) ==
		    NULL) {
			SEP_DRV_LOG_ERROR_TRACE_OUT(
				"Memory allocation failure!");
			return OS_NO_MEM;
	...
	for (i = 0; i < UNC_PCIDEV_num_entries(&(unc_pcidev_map[dev_node]));
		...
		if ((UNC_PCIDEV_busno_entry(&(unc_pcidev_map[dev_node]), i) ==
		     (S32)bus_no) &&
		    (UNC_PCIDEV_domainno_entry(&(unc_pcidev_map[dev_node]),
					       i) == (S32)domain_no)) {
	...
	if (i < entries) {
		UNC_PCIDEV_busno_entry(&(unc_pcidev_map[dev_node]), i) =
			(S32)bus_no;
		UNC_PCIDEV_domainno_entry(&(unc_pcidev_map[dev_node]), i) =
			(S32)domain_no;
		UNC_PCIDEV_num_entries(&(unc_pcidev_map[dev_node]))++;
		SEP_DRV_LOG_TRACE_OUT(
			"Add pkgno=%d num_packages=%d domainno=%x busno=%x devnode=%d.",
...
extern OS_STATUS UNC_COMMON_Init(void)
{
	...
	for (i = 0; i < MAX_DEVICES; i++) {
		SEP_DRV_MEMSET(&(unc_pcidev_map[i]), 0,
			       sizeof(UNC_PCIDEV_NODE));
	}

	SEP_DRV_MEMSET((char *)&uncore_topology, 0,
		       sizeof(UNCORE_TOPOLOGY_INFO_NODE));
	SEP_DRV_MEMSET((char *)&platform_topology_prog_node, 0,
		       sizeof(PLATFORM_TOPOLOGY_PROG_NODE));
#if defined(DRV_PMT_ENABLE)
	SEP_DRV_MEMSET((char *)&pmt_topology, 0,
	...
	for (i = 0; i < MAX_DEVICES; i++) {
		...
		if (UNC_PCIDEV_domainno_list(&(unc_pcidev_map[i]))) {
			UNC_PCIDEV_domainno_list(&(unc_pcidev_map[i])) =
				CONTROL_Free_Memory(UNC_PCIDEV_domainno_list(
					&(unc_pcidev_map[i])));
		}
 ...
 */
extern U32 UNC_COMMON_Scan_PMT_Device(void)
{
	U32 i = 0;
	U32 j = 0;
	U32 k = 0;
	...
	for (i = 0;
	     i < PMT_TOPOLOGY_DISCOVERY_LIST_num_pmt_devices(&pmt_topology);
	     i++) {
		for (j = 0; j < PMT_TOPOLOGY_DISCOVERY_LIST_num_tiles(
					&pmt_topology, i);
		     j++) {
			for (k = 0; k < pmt_dev_index; k++) {
				if (PMT_TOPOLOGY_DISCOVERY_LIST_tile_domain(
					    &pmt_topology, i, j) ==
					    pmt_devices[k].domain &&
				    //PMT_TOPOLOGY_DISCOVERY_LIST_tile_bus(&pmt_topology, i, j) == pmt_devices[k].bus &&
				    PMT_TOPOLOGY_DISCOVERY_LIST_tile_device(
					    &pmt_topology, i, j) ==
					...
					SEP_DRV_LOG_DETECTION(
						"PMT Device ndex=%i bus=%x dev=%x func=%x devid=%x venid=%x found\n",
						k, pmt_devices[k].bus,
						pmt_devices[k].device,
						pmt_devices[k].function,
						pmt_devices[k].device_id,
					...
					PMT_TOPOLOGY_DISCOVERY_LIST_num_entries_found(
						&pmt_topology, i)++;
					PMT_TOPOLOGY_DISCOVERY_LIST_tile_found(
						&pmt_topology, i, j) = 1;
					PMT_TOPOLOGY_DISCOVERY_LIST_tile_unit_id(
						&pmt_topology, i, j) = k;
					PMT_TOPOLOGY_DISCOVERY_LIST_tile_bus(
						&pmt_topology, i, j) =
						pmt_devices[k].bus;
					//PMT_TOPOLOGY_DISCOVERY_LIST_tile_id(&pmt_topology, i, j)    = i;
					SEP_DRV_LOG_DETECTION(
						"Found CTA device 0x%x at DBDF(%x:%x:%x:%x) [%u unit(s) so far].",
						pmt_devices[k].device_id,
						pmt_devices[k].domain,
						pmt_devices[k].bus,
						pmt_devices[k].device,
		...
		for (busno = 0; busno < MAX_BUSNO; busno++) {
			for (j = 0; j < MAX_PCI_DEVNO; j++) {
				...
				for (k = 0; k < MAX_PCI_FUNCNO; k++) {
					...
					device_found = 0;
					// Assume a single pci domain is connected with multiple sockets with consecutive package IDs
					// if the number of pci domains is smaller than the number of packages
					value = PCI_Read_U32_Valid(
						domainno, busno, j, k, 0, 0);
					CONTINUE_IF_NOT_GENUINE_INTEL_DEVICE(
						value, vendor_id, device_id);
					SEP_DRV_LOG_DETECTION(
						"Uncore deviceID=0x%x DBDF(%x:%x:%x:%x).",
						device_id, domainno, busno, j,
						k);
					...
					if (device_found) {
						if (UNC_COMMON_Add_Bus_Map(
							...
							UNCORE_TOPOLOGY_INFO_pcidev_num_entries_found(
								...
								k)++;
							SEP_DRV_LOG_DETECTION(
								"Found device 0x%x at DBDF(%x:%x:%x:%x) [%u unit(s) so far].",
								device_id,
								domainno, busno,
								j, k,
								UNCORE_TOPOLOGY_INFO_pcidev_num_entries_found(
 ...
 * <I>Special Notes:</I>
 */
extern VOID UNC_COMMON_PCI_Scan_For_CapId(UNCORE_DISCOVERY_DVSEC_CONFIG config,
		...
		for (busno = 0; busno < MAX_BUSNO; busno++) {
			for (j = 0; j < MAX_PCI_DEVNO; j++) {
				for (k = 0; k < MAX_PCI_FUNCNO; k++) {
					...
					while ((value = PCI_Read_U32_Valid(
							domainno, busno, j, k,
							offset, 0))) {
						next_offset =
							(value &
							 EXT_CAPID_LIST_NEXT_ADDR_MASK) >>
 ...
 * <I>Special Notes</I>
 */
static VOID unc_common_Read_MSR(PVOID param)
{
	...
	MSR_DATA_value(topo_msr) =
		(U64)SYS_Read_MSR((U32)MSR_DATA_addr(topo_msr));
 ...
 * <I>Special Notes</I>
 */
static U64 unc_common_Read_MSR_Package(U32 msr_address, U32 package_num)
{
	S32 i;
	U64 msr_value = 0ULL;
	...
	for (i = 0; i < GLOBAL_STATE_num_cpus(driver_state); i++) {
		if (package_num == core_to_package_map[i]) {
			...
			CONTROL_Invoke_Cpu(i, unc_common_Read_MSR,
					   (VOID *)&topo_msr_node);
...
extern VOID UNC_COMMON_Get_Platform_Topology(U32 dev_node)
{
	...
	U32 unc_topo_node =
		PLATFORM_TOPOLOGY_PROG_EXT_topology_device_device_index(
	...
	PLATFORM_TOPOLOGY_PROG_EXT_topology_device_prog_valid(
		&platform_topology_prog_node, dev_node) = 1;
	...
	num_registers =
		PLATFORM_TOPOLOGY_PROG_EXT_topology_device_num_registers(
			&platform_topology_prog_node, dev_node);
	...
	for (i = 0; i < num_pkgs; i++) {
		for (len = 0; len < num_registers; len++) {
			if (PLATFORM_TOPOLOGY_PROG_EXT_topology_device_distinct_buses(
				...
				reg_idx = PLATFORM_TOPOLOGY_REG_EXT_unit_index(
			...
			if (PLATFORM_TOPOLOGY_REG_EXT_reg_type(
				    topology_regs_ext, len) ==
			    PMU_REG_PROG_MSR) {
				reg_id = PLATFORM_TOPOLOGY_REG_EXT_reg_id(
					topology_regs_ext, len);

				// Validation check from PMU list
				if (!PMU_LIST_Check_MSR(reg_id)) {
					SEP_DRV_LOG_ERROR(
						"Received invalid MSR 0x%x. Skipped!",
						reg_id);
					continue;
				} else {
					SEP_DRV_LOG_TRACE(
				...
				if (PLATFORM_TOPOLOGY_PROG_EXT_topology_device_scope(
					    ...
					    dev_node) == PACKAGE_EVENT) {
					reg_value = unc_common_Read_MSR_Package(
						reg_id, i);
				} else {
					reg_value = SYS_Read_MSR(reg_id);
				}
				reg_mask = PLATFORM_TOPOLOGY_REG_EXT_reg_mask(
					topology_regs_ext, len);
				...
				SEP_DRV_LOG_DETECTION(
					...
					PLATFORM_TOPOLOGY_REG_EXT_reg_value(
						topology_regs_ext, len, i));
			} else if (PLATFORM_TOPOLOGY_REG_EXT_reg_type(
					   topology_regs_ext, len) ==
				   PMU_REG_PROG_PCI) {
				if (!IS_BUS_MAP_VALID(unc_topo_node, reg_idx)) {
					SEP_DRV_LOG_TRACE(
						"BUS MAP invalid for device=%d reg_idx=%d\n",
						dev_node, reg_idx);
					continue;
				}
				if (!PLATFORM_TOPOLOGY_REG_EXT_device_valid(
					    topology_regs_ext, len)) {
					SEP_DRV_LOG_TRACE(
						"Device=%d reg_idx=%d invalid\n",
						dev_node, reg_idx);
					continue;
				}
				bus_num = GET_BUS_MAP(unc_topo_node, reg_idx);
				domain_num =
					GET_DOMAIN_MAP(unc_topo_node, reg_idx);
				device_num = PLATFORM_TOPOLOGY_REG_EXT_device(
					topology_regs_ext, len);
				func_num = PLATFORM_TOPOLOGY_REG_EXT_function(
					topology_regs_ext, len);
				reg_offset = PLATFORM_TOPOLOGY_REG_EXT_reg_id(
					topology_regs_ext, len);

				// Validation check from PMU list
				if (!PMU_LIST_Check_PCI(
					    (U8)bus_num, (U8)device_num,
					    (U8)func_num, reg_offset)) {
					PLATFORM_TOPOLOGY_REG_EXT_device_valid(
						topology_regs_ext, len) = 0;
					SEP_DRV_LOG_ERROR(
						"Received invalid PCI device DBDF(%x:%x:%d:%d) Off=%x Skipped!",
						domain_num, bus_num, device_num,
						func_num, reg_offset);
					continue;
				} else {
					SEP_DRV_LOG_DETECTION(
						"Verified PCI dev_node=%d unc_topo_node=%d reg_idx=%d DBDF(%x:%x:%d:%d) Off=%x\n",
						dev_node, unc_topo_node,
						reg_idx, domain_num, bus_num,
						device_num, func_num,
						reg_offset);
				}

				device_value =
					PCI_Read_U32_Valid(domain_num, bus_num,
							   device_num, func_num,
							   0, 0);
				...
				if (PLATFORM_TOPOLOGY_REG_EXT_device_id(
					    topology_regs_ext, len) &&
				    PLATFORM_TOPOLOGY_REG_EXT_device_id(
					    topology_regs_ext, len) !=
					    device_id) {
					continue;
				}
				PLATFORM_TOPOLOGY_REG_EXT_reg_value(
					...
					PCI_Read_U32_Valid(domain_num, bus_num,
							   ...
							   reg_offset,
							   PCI_INVALID_VALUE);
				PLATFORM_TOPOLOGY_REG_EXT_bus_number(
					topology_regs_ext, len, i) =
					(U16)bus_num;
				PLATFORM_TOPOLOGY_REG_EXT_bus(
					topology_regs_ext, len) = (U16)bus_num;
				PLATFORM_TOPOLOGY_REG_EXT_domain_number(
					topology_regs_ext, len, i) =
					(U16)domain_num;
				SEP_DRV_LOG_DETECTION(
					"PCI topology DBDF(%x:%x:%d:%d) val=%llx\n",
					domain_num, bus_num, device_num,
					func_num,
					PLATFORM_TOPOLOGY_REG_EXT_reg_value(
						topology_regs_ext, len, i));
				if (i == 0 &&
				    PLATFORM_TOPOLOGY_REG_EXT_unit_index(
					    topology_regs_ext, len) >=
					    num_found_units) {
					num_found_units =
						PLATFORM_TOPOLOGY_REG_EXT_unit_index(
				...
				if (!PMU_LIST_Check_PCI(
					...
					PLATFORM_TOPOLOGY_REG_EXT_device_valid(
						topology_regs_ext, len) = 0;
					SEP_DRV_LOG_ERROR(
						"Received invalid MMIO device DBDF(%x:%x:%d:%d) Off=%llx Skipped!",
						domain_num, bus_num, device_num,
						func_num,
						PLATFORM_TOPOLOGY_REG_EXT_main_bar_offset(
							topology_regs_ext,
				...
				mmio_base1 =
					mmio_base1 &
					PLATFORM_TOPOLOGY_REG_EXT_main_bar_mask(
						topology_regs_ext, len);
				...
				physical_address =
					((mmio_base1
					  << PLATFORM_TOPOLOGY_REG_EXT_main_bar_shift(
						     topology_regs_ext, len)) |
					 (mmio_base2
					...
					PLATFORM_TOPOLOGY_REG_EXT_base_mmio_offset(
						topology_regs_ext, len);
				PCI_Map_Memory(
					&tmp_map, physical_address,
					PLATFORM_TOPOLOGY_REG_EXT_mmio_map_size(
						topology_regs_ext, len));
				virtual_address =
					SEP_MMIO_NODE_virtual_address(&tmp_map);
				PLATFORM_TOPOLOGY_REG_EXT_reg_value(
					topology_regs_ext, len, i) =
					PCI_MMIO_Read_U32(virtual_address,
							  reg_offset);
				PCI_Unmap_Memory(&tmp_map);
				SEP_DRV_LOG_DETECTION(
					"MMIO base1=%llx base2=%llx phys=%llx, virt=%llx\n",
					mmio_base1, mmio_base2,
					physical_address, virtual_address);
				SEP_DRV_LOG_DETECTION(
					"MMIO dev_node=%d unc_topo_node=%d val = %llx bus=%x\n",
					dev_node, unc_topo_node,
					PLATFORM_TOPOLOGY_REG_EXT_reg_value(
						topology_regs_ext, len, i),
					bus_num);
...
static void unc_common_Disc_Add_Allowlist_Key_Value(U64 base_addr, U64 offset)
{
	U64 key = 0;
	SEP_DRV_LOG_TRACE_IN("");
...
unc_common_Disc_Add_Allowlist_Entry(UNCORE_DISCOVERY_TABLE pkg_table)
{
	...
	for (i = 0; i < max_blocks; i++) {
		unit_entry = &UNCORE_DISCOVERY_TABLE_discovery_unit_node_entry(
			pkg_table, i);
		if (UNCORE_DISCOVERY_UNIT_unit_type(unit_entry) ==
			    DISCOVERY_UNIT_CXLCM ||
		    UNCORE_DISCOVERY_UNIT_unit_type(unit_entry) ==
			    DISCOVERY_UNIT_CXLDP) {
			unit_ctrl_addr = UNCORE_DISCOVERY_UNIT_unit_ctrl_addr(
				unit_entry);
			if (!unit_ctrl_addr) {
				continue;
			}
			...
			unc_common_Disc_Add_Allowlist_Key_Value(
				unit_ctrl_addr,
				UNCORE_DISCOVERY_UNIT_unit_status_offset(
					unit_entry));
			for (j = 0;
			     j < UNCORE_DISCOVERY_UNIT_num_gp_ctrs(unit_entry);
			     j++) {
				unc_common_Disc_Add_Allowlist_Key_Value(
					unit_ctrl_addr,
					(UNCORE_DISCOVERY_UNIT_evtsel0_offset(
						 unit_entry) +
					 j * 8));
				unc_common_Disc_Add_Allowlist_Key_Value(
					unit_ctrl_addr,
					(UNCORE_DISCOVERY_UNIT_ctr0_offset(
						 unit_entry) +
					 j * 8));
			}
		}
 ...
 *  <I>Special Notes:</I>
 */
extern VOID UNC_COMMON_Read_Uncore_Discovery_Tables(void)
{
	...
	SEP_DRV_MEMSET((char *)bar_list, 0,
		       (MAX_PACKAGES * sizeof(DRV_PCI_DEVICE_ENTRY_NODE)));
	config_node =
		UNCORE_DISCOVERY_dvsec_config_node(&uncore_discovery_tables);
	PCI_FIND_DVSEC_CAPABILITY_BAR(&config_node, bar_list);

	for (i = 0; i < num_packages; i++) {
		if (DRV_PCI_DEVICE_ENTRY_bar_address(&bar_list[i])) {
			pkg_table = &UNCORE_DISCOVERY_table_list_entry(
				&uncore_discovery_tables, i);
			SEP_DRV_MEMCPY(
				&UNCORE_DISCOVERY_TABLE_discovery_entry_node(
					pkg_table),
				&bar_list[i],
				sizeof(DRV_PCI_DEVICE_ENTRY_NODE));
			SEP_DRV_MEMSET((char *)&mmio_node, 0,
			...
			global_node = *(UNCORE_DISCOVERY_GLOBAL)(
				(char *)(UIOP)SEP_MMIO_NODE_virtual_address(
					&mmio_node));
			stride_size = (U32)UNCORE_DISCOVERY_GLOBAL_block_stride(
				&global_node);
			max_blocks = (U32)UNCORE_DISCOVERY_GLOBAL_max_blocks(
				&global_node);
			SEP_DRV_MEMCPY(
			...
			SEP_DRV_LOG_TRACE(
				"stride_size: %u, max_blocks: %u, gbl_ctrl_addr: 0x%llx",
			...
			if ((stride_size * 8) !=
				...
				SEP_DRV_LOG_WARNING(
					"Discovery unit stride %u unmatched size of UNCORE_DISCOVERY_UNIT_NODE %u",
					(stride_size * 8),
					sizeof(UNCORE_DISCOVERY_UNIT_NODE));
				return;
			}
			if ((sizeof(UNCORE_DISCOVERY_UNIT_NODE) *
			     MAX_DISCOVERY_UNCORE_UNITS) <
			    (max_blocks * stride_size * 8)) {
				SEP_DRV_LOG_WARNING(
					"Discovery table size %u greater than allocated memory in kernel %u",
					(max_blocks * stride_size * 8),
					sizeof(UNCORE_DISCOVERY_UNIT_NODE) *
						MAX_DISCOVERY_UNCORE_UNITS);
				return;
			...
			PCI_Map_Memory(
				...
				(max_blocks * stride_size * 8));
			SEP_DRV_MEMCPY(
				UNCORE_DISCOVERY_TABLE_discovery_unit_node(
					pkg_table),
				(char *)(UIOP)SEP_MMIO_NODE_virtual_address(
					&mmio_node),
				(max_blocks * stride_size * 8));
