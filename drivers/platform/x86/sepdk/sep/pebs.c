...
)
{
	...
	DTS_BUFFER_EXT_pebs_threshold(dts)  = DTS_BUFFER_EXT_pebs_base(dts) +
		(LWPMU_DEVICE_pebs_record_size(&devices[dev_idx]) * DEV_CONFIG_pebs_record_num(pcfg));
...
)
{
	...
	pebs_ptr       = (S8 *)((UIOP)DTS_BUFFER_EXT_pebs_base(dtes) +
			(rec_index * LWPMU_DEVICE_pebs_record_size(&devices[dev_idx])));
...
)
{
	...
	FOR_EACH_DATA_REG(pecb, j) {
		...
		psamp_pebs = (SampleRecordPC *)OUTPUT_Reserve_Buffer_Space(bd, EVENT_DESC_sample_size(evt_desc),
							(NMI_mode)? TRUE:FALSE, !SEP_IN_NOTIFICATION, (S32)this_cpu);
		if (!psamp_pebs) {
