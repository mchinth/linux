 ...
 *
 */
static void perfver4_Read_PMU_Data(PVOID param, U32 dev_idx)
{
	U32 j;
	...
	FOR_EACH_REG_CORE_OPERATION(pecb, i, PMU_OPERATION_DATA_ALL)
	{
		...
		if (overflow_status & ((U64)1 << index)) {
			...
			if (ECB_entries_collect_on_ctx_sw_get(pecb, i)) {
				DRV_EVENT_MASK_collect_on_ctx_sw(&event_flag) =
					1;
			}
	...
	FOR_EACH_REG_CORE_OPERATION(pecb, i, PMU_OPERATION_DATA_GP)
	{
		if (ECB_entries_ebc_sampling_evt_get(pecb, i) ||
			...
			CPU_STATE_em_tables(pcpu)[index] =
				SYS_Read_MSR(ECB_entries_reg_id(pecb, i));
			SEP_DRV_LOG_TRACE(
				"Saved value for cur grp %u, reg 0x%x : 0x%llx.",
				CPU_STATE_current_group(pcpu),
				ECB_entries_reg_id(pecb, i),
				CPU_STATE_em_tables(pcpu)[index]);
		}
	...
	FOR_EACH_REG_CORE_OPERATION(pecb, i, PMU_OPERATION_DATA_GP)
	{
		if (ECB_entries_ebc_sampling_evt_get(pecb, i) ||
			...
			SYS_Write_MSR(ECB_entries_reg_id(pecb, i),
				      CPU_STATE_em_tables(pcpu)[index]);
			SEP_DRV_LOG_TRACE(
				"Restore value for next grp %u, reg 0x%x : 0x%llx.",
				next_group, ECB_entries_reg_id(pecb, i),
				CPU_STATE_em_tables(pcpu)[index]);
		}
	}
	END_FOR_EACH_REG_CORE_OPERATION;
 ...
 *
 */
static U64 perfver4_Read_LBRs(VOID *buffer)
{
	U32 i, count = 0;
	U64 *lbr_buf = NULL;
	U64 value = 0;
...
static VOID perfver4_Clean_Up(VOID *param)
{
	...
	for (j = 0; j < LWPMU_DEVICE_em_groups_count(&devices[dev_idx]); j++) {
		(pecb_first_grp) =
			LWPMU_DEVICE_PMU_register_data(&devices[dev_idx])[j];
		if (!(pecb_first_grp)) {
			continue;
		} else {
			break;
	...
	if (pecb_first_grp) {
		SYS_Write_MSR(
			ECB_entries_reg_id(pecb_first_grp,
					   ECB_SECTION_REG_INDEX(
						   pecb_first_grp,
