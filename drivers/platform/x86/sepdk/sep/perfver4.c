...
)
{
	 ...
	 */
	if (DRV_SETUP_INFO_pebs_accessible(&req_drv_setup_info) && i == ECB_SECTION_REG_INDEX(pecb, PEBS_ENABLE_REG_INDEX, PMU_OPERATION_GLOBAL_REGS) && ECB_entries_reg_value(pecb, i)) {
	    SYS_Write_MSR(ECB_entries_reg_id(pecb, i), 0LL);
	    continue;
	}

	if (DEV_CONFIG_pebs_mode(pcfg) && (ECB_entries_precise_get(pecb, i) == 1)) {
	    PEBS_Reset_Counter(this_cpu, i, PMU_OPERATION_ALL_REG);
	}
...
#if defined(MYDEBUG)
	{
	    ...
	    SEP_DRV_LOG_TRACE("Write reg 0x%x --- value 0x%llx -- read 0x%llx.", ECB_entries_reg_id(pecb, i), ECB_entries_reg_value(pecb, i), val);
	}
#endif
    } END_FOR_EACH_REG_CORE_OPERATION;
...
)
{
	...
	if (DRV_CONFIG_ipt_mode(drv_cfg)) {
	    SEP_DRV_LOG_TRACE_OUT("Exit for IPT collection");
	    return;
	}

	/* Clear outstanding frozen bits */
	SYS_Write_MSR(ECB_entries_reg_id(pecb, ECB_SECTION_REG_INDEX (pecb, GLOBAL_OVF_CTRL_REG_INDEX, PMU_OPERATION_GLOBAL_REGS)), PERFVER4_FROZEN_BIT_MASK);
	...
	if (CPU_STATE_reset_mask(pcpu)) {
	    ...
	    SYS_Write_MSR(ECB_entries_reg_id(pecb, ECB_SECTION_REG_INDEX(pecb, GLOBAL_CTRL_REG_INDEX, PMU_OPERATION_GLOBAL_REGS)), ECB_entries_reg_value(pecb, ECB_SECTION_REG_INDEX(pecb, GLOBAL_CTRL_REG_INDEX, PMU_OPERATION_GLOBAL_REGS)));
	    SYS_Write_MSR(ECB_entries_reg_id(pecb, ECB_SECTION_REG_INDEX(pecb, DEBUG_CTRL_REG_INDEX, PMU_OPERATION_GLOBAL_REGS)), ECB_entries_reg_value(pecb, ECB_SECTION_REG_INDEX(pecb, DEBUG_CTRL_REG_INDEX, PMU_OPERATION_GLOBAL_REGS)));
	...
	if (CPU_STATE_group_swap(pcpu)) {
		...
		SYS_Write_MSR(ECB_entries_reg_id(pecb, ECB_SECTION_REG_INDEX(pecb, PEBS_ENABLE_REG_INDEX, PMU_OPERATION_GLOBAL_REGS)), ECB_entries_reg_value(pecb, ECB_SECTION_REG_INDEX(pecb, PEBS_ENABLE_REG_INDEX, PMU_OPERATION_GLOBAL_REGS)));
	    }
	    SYS_Write_MSR(ECB_entries_reg_id(pecb, ECB_SECTION_REG_INDEX(pecb, DEBUG_CTRL_REG_INDEX, PMU_OPERATION_GLOBAL_REGS)), ECB_entries_reg_value(pecb, ECB_SECTION_REG_INDEX(pecb, DEBUG_CTRL_REG_INDEX, PMU_OPERATION_GLOBAL_REGS)));
#if defined(MYDEBUG)
	    {
		...
		SEP_DRV_LOG_TRACE("Write reg 0x%x--- read 0x%llx.", ECB_entries_reg_id(pecb, 0), val);
	    }
#endif
	}
	...
	if (multi_pebs_enabled) {
	    ...
	    if (IS_FIXED_CTR_ENABLED(global_control_val) || !IS_PMC_PEBS_ENABLED_GP(global_control_val, pebs_enable_val)) {
		SEP_DRV_LOG_TRACE("Global_control_val = 0x%llx pebs_enable_val = 0x%llx.", global_control_val, pebs_enable_val);
		SYS_Write_MSR(ECB_entries_reg_id(pecb, ECB_SECTION_REG_INDEX(pecb, DEBUG_CTRL_REG_INDEX, PMU_OPERATION_GLOBAL_REGS)), DISABLE_FRZ_ON_PMI(ECB_entries_reg_value(pecb, ECB_SECTION_REG_INDEX(pecb, DEBUG_CTRL_REG_INDEX, PMU_OPERATION_GLOBAL_REGS))));
	    }
	}
    }
...
)
{
	...
	j = EMON_BUFFER_CORE_EVENT_OFFSET(EMON_BUFFER_DRIVER_HELPER_core_index_to_thread_offset_map(emon_buffer_driver_helper)[this_cpu], ECB_entries_core_event_id(pecb, i));
	...
	if (ECB_entries_fixed_reg_get(pecb, i)) {
		...
		j = EMON_BUFFER_CORE_EVENT_OFFSET(EMON_BUFFER_DRIVER_HELPER_core_index_to_thread_offset_map(emon_buffer_driver_helper)[this_cpu], EMON_BUFFER_DRIVER_HELPER_core_num_events(emon_buffer_driver_helper));
...
)
{
    ...
    if (DEV_CONFIG_pebs_mode(pcfg) && (DEV_CONFIG_pebs_record_num(pcfg) == 1)) {
	overflow_status = PEBS_Overflowed (this_cpu, overflow_status, 0);
    }
    overflow_status_clr = overflow_status;

    if (DRV_CONFIG_ipt_mode(drv_cfg)) {
	IPT_TOPA_Interrupted(this_cpu, overflow_status);
	overflow_status_clr &= ~PERF_GLOBAL_STATUS_TOPA_PMI_MASK;
    }
	...
	if (overflow_status & ((U64)1 << index)) {
	    ...
	    SEP_DRV_LOG_TRACE("Register 0x%x --- val 0%llx.", ECB_entries_reg_id(pecb, i), SYS_Read_MSR(ECB_entries_reg_id(pecb, i)));
	    SYS_Write_MSR(ECB_entries_reg_id(pecb, i), ECB_entries_reg_value(pecb, i));
	    ...
	    SEP_DRV_LOG_TRACE("Slot# %d, reg_id 0x%x, index %d.", i, ECB_entries_reg_id(pecb, i), index);
    ...
    SYS_Write_MSR(ECB_entries_reg_id(pecb, ECB_SECTION_REG_INDEX(pecb, GLOBAL_OVF_CTRL_REG_INDEX, PMU_OPERATION_GLOBAL_REGS)), overflow_status_clr & PERFVER4_OVERFLOW_BIT_MASK_HT_ON);
...
)
{
    ...
    FOR_EACH_REG_CORE_OPERATION(pecb, i, PMU_OPERATION_DATA_GP) {
	if (ECB_entries_ebc_sampling_evt_get(pecb, i) || !DRV_CONFIG_event_based_counts(drv_cfg)) {
	    index = st_index + i - ECB_operations_register_start(pecb, PMU_OPERATION_DATA_GP);
	    CPU_STATE_em_tables(pcpu)[index] = SYS_Read_MSR(ECB_entries_reg_id(pecb, i));
	    SEP_DRV_LOG_TRACE("Saved value for cur grp %u, reg 0x%x : 0x%llx.", CPU_STATE_current_group(pcpu), ECB_entries_reg_id(pecb, i), CPU_STATE_em_tables(pcpu)[index]);
	}
    } END_FOR_EACH_REG_CORE_OPERATION;
	...
	FOR_EACH_REG_CORE_OPERATION(pecb, i, PMU_OPERATION_DATA_ALL) {
	    if (!ECB_entries_ebc_sampling_evt_get(pecb, i) && (ECB_entries_event_id_index(pecb, i) != CPU_STATE_trigger_event_num(pcpu))) {
		SYS_Write_MSR(ECB_entries_reg_id(pecb, i), 0LL);
	    }
	} END_FOR_EACH_REG_CORE_OPERATION;
    }
	...
	if (ECB_entries_ebc_sampling_evt_get(pecb, i) || !DRV_CONFIG_event_based_counts(drv_cfg)) {
	    ...
	    SEP_DRV_LOG_TRACE("Restore value for next grp %u, reg 0x%x : 0x%llx.", next_group, ECB_entries_reg_id(pecb, i), CPU_STATE_em_tables(pcpu)[index]);
	}
    } END_FOR_EACH_REG_CORE_OPERATION;
	...
	FOR_EACH_REG_CORE_OPERATION(pecb, i, PMU_OPERATION_DATA_ALL) {
		...
		PEBS_Reset_Counter(this_cpu, i, PMU_OPERATION_DATA_ALL);
	    }
	} END_FOR_EACH_REG_CORE_OPERATION;
    }
...
)
{
    ...
    /* Clear outstanding frozen bits */
    if (pecb_first_grp) {
	SYS_Write_MSR(ECB_entries_reg_id(pecb_first_grp, ECB_SECTION_REG_INDEX(pecb_first_grp, GLOBAL_OVF_CTRL_REG_INDEX, PMU_OPERATION_GLOBAL_REGS)), PERFVER4_FROZEN_BIT_MASK);
    }
...
)
{
    ...
    CPU_STATE_reset_mask(pcpu) = overflow_status_clr;
    /* Clear outstanding overflow bits */
    SYS_Write_MSR(ECB_entries_reg_id(pecb, ECB_SECTION_REG_INDEX(pecb, GLOBAL_OVF_CTRL_REG_INDEX, PMU_OPERATION_GLOBAL_REGS)), overflow_status_clr & PERFVER4_OVERFLOW_BIT_MASK_HT_OFF);
...
)
{
    ...
    CPU_STATE_reset_mask(pcpu) = overflow_status_clr;
    /* Clear outstanding overflow bits */
    SYS_Write_MSR(ECB_entries_reg_id(pecb, ECB_SECTION_REG_INDEX(pecb, GLOBAL_OVF_CTRL_REG_INDEX, PMU_OPERATION_GLOBAL_REGS)), overflow_status_clr & PERFVER4_OVERFLOW_BIT_MASK_NON_HT);
...
)
{
	...
	*data = CPU_STATE_current_group(pcpu) + 1;
	...
	if (event_id == id) {
	    *data = ~(ECB_entries_reg_value(pecb, i) - 1) & ECB_entries_max_bits(pecb, i);;
	} else {
